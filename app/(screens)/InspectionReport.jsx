...
export default function InspectionReport() {
  ...
  const renderReportItem = ({item}) => {
    ...
    const inspectionDetails = selectedInspections?.find(i => i.id === item.inspectionId);
    ...
    if (template && template.inspectionItems) {
      templateItems = selectedItems?.filter(i => 
        template.inspectionItems.includes(i.id)
      ) || [];
    }

    // Combine all relevant items
    const allItems = [...items, ...templateItems].filter((item, index, self) => 
        ...
        <View style={styles.reportCard}>
          <View style={styles.reportHeader}>
            <Text style={styles.reportTitle}>
              {item.title || item.inspectionType || template?.name || 'Inspection Report'}
            </Text>
            {item.status && (
              <View
          ...
          <Text style={styles.reportSubtitle}>
            {property?.name || property?.address || item.propertyName || item.property || 'Property Not Found'}
          </Text>
          <Text style={styles.reportDate}>
            {item.date || item.timestampFormatted || new Date(item.timestamp?.seconds * 1000).toLocaleDateString()}
          </Text>
